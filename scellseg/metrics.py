...
def evaluate_predictions_on_coco(coco_gt, coco_dt, iou_type, kpt_oks_sigmas=None, use_fast_impl=False):
    """
    based heavily on function from https://github.com/sartorius-research/LIVECell/blob/main/code/coco_evaluation.py
   ...
   # print("_evaluate_predictions_on_coco")
            ...
            def _summarize(ap=1, iouThr=None, areaRng='all', maxDets=2000):
                ...
                if ap == 1:
                    ...
                    if iouThr is not None:
                        t = np.where(iouThr == np.around(p.iouThrs, 2))[0]
                        s = s[t]
                    s = s[:, :, :, aind, mind]
                else:
    ...
    if iou_type == "keypoints":
        num_keypoints = len(coco_dt[0]["keypoints"]) // 3
...
def derive_coco_results(coco_eval, iou_type, class_names=None):
    """
    From https://github.com/sartorius-research/LIVECell/blob/main/code/coco_evaluation.py
    ...
    metrics = {
        ...
        "segm": ["AP", "AP50", "AP55", "AP60", "AP65", "AP70", "AP75", "AP80", "AP85", "AP90", "AP95"],
