...
class DatasetShot(Dataset):
    ...
    def __getitem__(self, i):
        ...
        # #  #2. 弹性变换
        # if i>0 and i%4==0:
        #     shot_image, shot_mask = random_elastic(shot_image, shot_mask, alpha_w=3, sigma_w=0.08, alpha_affine_w=0.08)
            # is_elastic = True

        # 2.5 pad image
        # shot_image, shot_mask = pad_image(shot_image, shot_mask)

        # 3. 随机切出224的区域
        # shot_mask_raw = cp.deepcopy(shot_mask)

        # clip_size = np.random.choice(np.array([112, 224]))
        # clip_size = 224
        # # # if i<400:
        # # #     clip_size = 224
        # # # else:
        # # #     clip_size = 128
        # if self.multi_class:
        #     shot_image, shot_mask, shot_class = random_clip(X=shot_image, M=shot_mask, C=shot_class, xy=(clip_size, clip_size))
        # else:
        #     shot_image, shot_mask = random_clip(X=shot_image, M=shot_mask, xy=(clip_size, clip_size))

        # shot_mask = mask_quality_contorl(shot_mask, shot_mask_raw)  # 去除被切碎的mask
        # # mask_old_path = os.path.join(save_dir, 'mask_old'+str(i)+'.png')
        # # mask_new_path = os.path.join(save_dir, 'mask_new'+str(i)+'.png')
        # # imsave(mask_old_path, shot_mask_raw[0])
        # # imsave(mask_new_path, shot_mask[0])

        # 4. 从图中随机进行采样, 会使背景变黑
        # if i>0 and i%4==0 and not is_elastic:
        #     percent = np.random.choice(np.array([0.2]))
        #     # # percent = np.random.choice(np.array([0.8, 1]))
        #     shot_image_raw = cp.deepcopy(shot_image)
        #     shot_mask_raw = cp.deepcopy(shot_mask)
        #     shot_image, shot_mask = random_sample_cell(shot_image_raw, shot_mask_raw, percent=percent)

        # shot_lbl = None
        # # 4.5 转换成flow
        # if self.task_mode == 'cellpose':
        #     shot_flow = dynamics.labels_to_flows(shot_mask)
        #     shot_lbl = [shot_flow[0][1:]]  # (3, ...)
        # elif self.task_mode == 'classic-3':
        #     shot_lbl0 = [np.stack((label, label > 0, distance_to_boundary(label)), axis=0).astype(np.float32) for label in shot_mask]
        #     shot_lbl = [shot_lbl0[0][1:]]  # (2, ...)
        #     unet=True
        # elif self.task_mode == 'classic-2':
        #     shot_lbl0 = [np.stack((label, label > 0), axis=0).astype(np.float32) for label in shot_mask]
        #     shot_lbl = [shot_lbl0[0][1:]]  # (1, ...)
        #     unet=True
        # elif self.task_mode == 'hover':
        #     shot_flow = dynamics.labels_to_hovers(shot_mask)
        #     shot_lbl = [shot_flow[0][1:]]  # (3, ...)
        # elif self.task_mode == 'cellpose_hover':
        #     shot_cellpose = dynamics.labels_to_flows(shot_mask)
        #     shot_hover = dynamics.labels_to_hovers(shot_mask)
        #     shot_lbl = [np.concatenate((shot_cellpose[0][1:], shot_hover[0][2:]), axis=0).astype(np.float32)]
        ...
        if i>=0:  # and i%2==0
            ...
            # rsc = np.array([md / self.diam_mean], np.float32)
            if self.multi_class:
        ...
        # 6. 保存shot image
        # save_shot_dir = r'G:\Python\9-Project\1-flurSeg\scellseg\output\shots\img'
        # image_path = os.path.join(save_shot_dir, 'img'+str(i)+'.png')
        # if shot_image[0].ndim == 3:
        #     if shot_image[0].shape[0] == 2:
        #         save_shot_image = np.concatenate((shot_image[0], np.zeros((1,224,224))))
        #         save_shot_image = save_shot_image.astype(np.float32)
        #     else:
        #         save_shot_image = shot_image[0]
        #     # imsave(image_path, save_shot_image.transpose((1,2,0)))
        #     imsave(image_path, save_shot_image[0])
        # else:
        #     imsave(image_path, shot_image[0])
        save_shot_lbl_dir = r'G:\Python\9-Project\1-flurSeg\scellseg\output\shots\lbl'
        lbl_path = os.path.join(save_shot_lbl_dir, 'lbl'+str(i)+'.png')
        imsave(lbl_path, shot_mask[0])
...
class DatasetQuery(Dataset):
    ...
    def __len__(self):
        return len(self.query_image_names)  # 只返回query的数量
...
class DatasetTsne(Dataset):
    """The class to load the mdataset"""

    def __init__(self, eval_dir=None, class_name=None, image_filter='_img', mask_filter='_masks'):
        ...
        for i in range(len(negative_dir)):
            ...
            class_ids+=[class_id]*len(negative_img_namesi)
            image_names += negative_img_namesi
            negative_maski_names = get_label_files(negative_img_namesi, mask_filter, imf=image_filter)[0]
            negative_mask_names += negative_maski_names
            label_names += negative_maski_names
            class_id += 1

        diams = []  # 计算cellpose中数据集的每张图片的md
        for negative_mask_name in negative_mask_names:
            negative_maski = imread(negative_mask_name)
            md = diameters(negative_maski)[0]
            if md < 5.0:
                md = 5.0
            diams.append(md)
        ...
        if eval_dir is not None:
            ...
            query_path = osp.join(eval_dir, 'query')
            query_image_names = get_image_files(query_path, mask_filter, image_filter)
            image_names += query_image_names
            label_names += get_label_files(query_image_names, mask_filter, imf=image_filter)[0]
            class_ids += ([class_id]*len(query_image_names))
            self.test_num = len(query_image_names)
        else:
            self.test_num = 0
...
if __name__ == '__main__':
    ...
    evalset = DatasetTsne(dataset_dir, class_name=None, image_filter='_img', mask_filter='_masks')
