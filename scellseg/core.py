...
try:
    ...
    from . import net_desc_nbranch as net_desc_nbranch
    # from . import net_desc_nbranch as net_desc
    TORCH_ENABLED = True
...
class UnetModel():
    def __init__(self, gpu=False, pretrained_model=False,
                    ...
                    residual_on=False, style_on=False, concatenation=True, update_step=1,
        ...
        self.net.multi_class = multi_class
        self.net.use_branch = use_branch
        self.net.ndecoder = ndecoder
        self.net.ntasker_seg = ntasker_seg
        self.net.last_conv_on = last_conv_on
    ...
    def _run_tiled(self, imgi, augment=False, bsize=224, tile_overlap=0.1):
        ...
        if imgi.ndim==4:  # 3D图片，有z-stack情况下
            ...
            IMG, ysub, xsub, Ly, Lx = transforms.make_tiles(imgi[0], bsize=bsize, 
                                                            augment=augment, tile_overlap=tile_overlap)  # 这里获取了z-stack中的一张,这里有aug
            ny, nx, nchan, ly, lx = IMG.shape
            batch_size *= max(4, (bsize**2 // (ly*lx))**0.5)  # 在bsize的情况下，batch_size最大这么大，图片更小时，batch_size可以更大
    ...
    def train(self, train_data, train_labels, train_files=None,
        ...
        # cell_threshold, boundary_threshold = self.threshold_validation(val_data, val_classes)
    ...
    def _set_optimizer(self, learning_rate, momentum, weight_decay, use_adam=False):
        if use_adam:
    ...
    def _set_criterion(self):
        if self.unet or ('classic' in self.task_mode):  # 在cellposeModel中会被关掉
    ...
    def _train_net(self, train_data, train_labels, 
              ...
              pretrained_model=None, save_path=None, save_every=100, use_adam=False,
        ...
        if rescale:
            diam_train = np.array([utils.diameters(train_labels[k][0])[0] for k in range(len(train_labels))])  # 求出每个训练图像的平均直径
