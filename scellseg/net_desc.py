...
class Extractor(nn.Module):
    def __init__(self, nbase, sz, residual_on=True, mkldnn=False, style_on=True,
    ...
    def forward(self, data, use_upsample=True):
        ...
        # for i in range(4):
        #     b, c, h, w = T0[i].size()
        #     t0 = T0[i].view(b, c, h*w).float()
        #     style_gram = torch.bmm(t0, t0.transpose(1, 2))
        #     style_gram = style_gram / torch.sum(style_gram ** 2, axis=1, keepdim=True) ** .5
        #     styles_gram.append(style_gram)

            # style_vec = style_gram[:, 0]
            # for i in range(1, style_gram.shape[1]):
            #     style_vec = torch.cat((style_vec, style_gram[:, i, i:]), 1)  # gram矩阵的压缩版的向量，表示纹理信息
            # styles_vec.append(style_vec)
...
class sCSnet(nn.Module):
    def __init__(self, nbase, nout, sz,
                 ...
                 attn_on=False, dense_on=False, phase='eval', multi_class=False,
        ...
        self.extractor = Extractor(nbase, sz, residual_on=residual_on, mkldnn=self.mkldnn, style_on=style_on,
             concatenation=concatenation, attn_on=attn_on, dense_on=dense_on)  # TODO：加self后续可以改变，不加仅在初始化的时候可以改变
        self.tasker = Tasker(nbase[1], nout, mkldnn=self.mkldnn, task_mode=task_mode)
    ...
    def shot_train_forward(self, data_shot, label_shot):
        for _ in range(0, self.update_step):
            ...
            logits = self.tasker(embedding_shot)
    ...
    def eval_forward(self, data):
        ...
        logits_q = self.tasker(embedding)
        if self.multi_class:
