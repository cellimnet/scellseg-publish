...
from scellseg.attention.se_attn import SEAttention
from scellseg.attention.cbam_attn import CBAMBlock
...
class brcStyle(nn.Module):
    def __init__(self, in_channels, out_channels, style_channels, sz, concatenation=False):
        super().__init__()
        self.conv = brcBlock(in_channels, out_channels, sz)  # style的开关不是在这里
        ...
        if concatenation:
            ...
            self.attn = CBAMBlock(out_channels, 8, kernel_size=1)

    def forward(self, style, x, mkldnn=False):
        feat = self.full(style)  # 这里才是style的层，模型关掉style的话这里全是0
        if mkldnn:
            x = x.to_dense()
        ...
        # style = self.full(style)  # 这里才是style的层，模型关掉style的话这里全是0
        # style = style.unsqueeze(-1).unsqueeze(-1)
        # feat = self.attn(style)  # 这里才是style的层，模型关掉style的话这里全是0
        # if mkldnn:
        #     x = x.to_dense()
        #     feat = style.to_dense()
        # y = x + feat
...
class resup(nn.Module):
    ...
    def forward(self, x, y, style, mkldnn=False):
        ...
        if self.dense_on:
            # out = self.up_conv(x)+out
            out = self.up_conv(torch.cat([x, out], dim=1))  # 第二种方案
        return out
...
class upsample(nn.Module):
    def __init__(self, nbase, sz, residual_on=True, concatenation=False, attn_on=False, dense_on=False, style_channels=[256, 256, 256, 256]):
        ...
        for n in range(1, len(nbase)-1):
            ...
            if attn_on:
                self.up_conv.add_module('up_conv_%d' % (n - 1), cbrBlock(nbase[n], nbase[n-1], 1))  # 这边将默认的sz=3换成了1，只是为了降维
    ...
    def forward(self, style, xd, mkldnn=False):
        ...
        for n in range(len(self.up) - 2, -1, -1):
            ...
            if self.attn_on:
                x = self.up_conv[n](x)
                xdi = self.attn[n](x, xd[n])
                x = torch.cat((xdi, x), dim=1)
                y_w=0  # 这里已经连接上了xd[n]信息，所以不再传入

            x = self.up[n](x, xd[n]*y_w, style[n], mkldnn=mkldnn)  # xd是下采样过程中每个块的特征
        return x
