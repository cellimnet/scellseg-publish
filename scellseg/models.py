...
class sCellSeg(UnetModel):
    ...
    def __init__(self, gpu=False, pretrained_model=False,
        ...
        # nclasses: default 3; prediction maps (dY, dX and cellprob)
        if task_mode=='classic-2':
            nclasses = 2

        self.nclasses = nclasses

        # if pretrained_model:  # TODO: 后期做用户逻辑整理一下
        ...
        assert ndecoder<=2; 'sorry, Not currently supported, you need refine the net_desc_nbranch.py manually'
        assert ntasker_seg<=2; 'sorry, Not currently supported, you need refine the net_desc_nbranch.py manually'
        if ndecoder>=2 or ntasker_seg>=2:
            use_branch = True
        if ndecoder>=2:
            ntasker_seg=1
            if multi_class: ndecoder+=1
        if ntasker_seg>=2:
            ndecoder=1
    ...
    def set_optimizer(self, lr, multi_class=False, use_branch=False, ndecoder=1, ntasker_seg=1):
        if use_branch:
                ...
                if multi_class:
                    optimizer = torch.optim.Adam(
                        [{'params': filter(lambda p: p.requires_grad, self.net.extractor.downsample.parameters()), 'key_name': 'downsample'},
                         ...
                         {'params': self.net.tasker0.parameters(), 'lr': lr['tasker0'], 'key_name': 'tasker0'},
                         {'params': self.net.tasker1.parameters(), 'lr': lr['tasker1'], 'key_name': 'tasker1'},
                         {'params': self.net.tasker_class.parameters(), 'lr': lr['tasker_class'], 'key_name': 'tasker_class'},], lr=lr['downsample'])
                else:
                    optimizer = torch.optim.Adam(
                        [{'params': filter(lambda p: p.requires_grad, self.net.extractor.downsample.parameters()), 'key_name': 'downsample'},
                         {'params': self.net.extractor.upsample.parameters(), 'lr': lr['upsample'], 'key_name': 'upsample'},
                         {'params': self.net.tasker0.parameters(), 'lr': lr['tasker0'], 'key_name': 'tasker0'},
                         {'params': self.net.tasker1.parameters(), 'lr': lr['tasker1'], 'key_name': 'tasker1'}], lr=lr['downsample'])
            else:
                if multi_class:
                    optimizer = torch.optim.Adam(
                        [{'params': filter(lambda p: p.requires_grad, self.net.extractor.downsample.parameters()), 'key_name': 'downsample'},
                         {'params': self.net.extractor.upsample.parameters(), 'lr': lr['upsample'], 'key_name': 'upsample'},
                         {'params': self.net.tasker_seg.parameters(), 'lr': lr['tasker_seg'], 'key_name': 'tasker_seg'},
                         {'params': self.net.tasker_class.parameters(), 'lr': lr['tasker_class'], 'key_name': 'tasker_class'}], lr=lr['downsample'])
                else:
                    optimizer = torch.optim.Adam(
                        [{'params': filter(lambda p: p.requires_grad, self.net.extractor.downsample.parameters()), 'key_name': 'downsample'},
                         {'params': self.net.extractor.upsample.parameters(), 'lr': lr['upsample'], 'key_name': 'upsample'},
                         {'params': self.net.tasker_seg.parameters(), 'lr': lr['tasker_seg'], 'key_name': 'tasker_seg'}], lr=lr['downsample'])
        else:
            optimizer = torch.optim.Adam(
                [{'params': filter(lambda p: p.requires_grad, self.net.extractor.downsample.parameters()), 'key_name': 'downsample'},
                 {'params': self.net.extractor.upsample.parameters(), 'lr': lr['upsample'], 'key_name': 'upsample'},
    ...
    def lr_step(self, now_step, step_size, threshold=0.0005):
        if now_step > 0:
    ...
    def train(self, train_data, train_labels, train_files=None,
        ...
        model_path = self._train_net(train_data, train_lbls,
                                     test_data, test_lbls,
                                     pretrained_model, save_path, save_every, use_adam,
                                     learning_rate, n_epochs, momentum, weight_decay, batch_size, rescale)

        # find threshold using validation set
        if 'classic' in self.task_mode:
    ...
    def train2(self, train_data, train_labels, train_files=None,
              test_data=None, test_labels=None, test_files=None,
              channels=None, normalize=True, pretrained_model=None,
              save_path=None, save_every=100, use_adam=False,
              learning_rate=0.2, n_epochs=500, momentum=0.9, weight_decay=0.00001, batch_size=8, rescale=True):
        ...
        log_dir = os.path.join(r'G:\Python\9-Project\1-flurSeg\scellseg\output\train_log', d.strftime("%Y%m%d-%H%M"))
        ...
        for iepoch in range(self.n_epochs):
            ...
            if iepoch % 10 == 0 or iepoch < 10:  # 测试前10批和10的整数倍的
                ...
                if test_data is not None:
                    ...
                    log_writer.add_scalars('train_scellstyle', {'train_loss': lavg, 'val_loss': lavgt / nsum}, iepoch)
                    print('Epoch %d, Time %4.1fs, Loss %2.4f, Loss Test %2.4f, LR %2.4f' %
                          (iepoch, time.time() - tic, lavg, lavgt / nsum, LR[iepoch]))
                else:
                    log_writer.add_scalars('train_scellstyle', {'train_loss': lavg}, iepoch)
    ...
    def train3(self, train_data, train_labels, train_files=None,
              test_data=None, test_labels=None, test_files=None,
              channels=None, normalize=True, pretrained_model=None,
              save_path=None, save_every=100, use_adam=False,
              learning_rate=0.2, n_epochs=500, momentum=0.9, weight_decay=0.00001, batch_size=8, rescale=True):
        ...
        self.net.alpha = torch.tensor(1., requires_grad=True)
        self.net.optimizer = torch.optim.SGD(self.net.parameters(), lr=learning_rate, momentum=momentum, weight_decay=weight_decay)        # set learning rate schedule
    ...
    def query_eval(self, shot_gen=None, query_image_names=None, query_images=None, eval_batch_size=8,
            channel=None, diameter=None, normalize=True, invert=False,
            augment=False, tile=True, tile_overlap=0.5,
            resample=False, interp=True,
            do_3D=False, anisotropy=None, stitch_threshold=0,
            postproc_mode='cellpose', lr =None, lr_schedule_gamma=None, step_size=20,
            flow_threshold=0.4, cellprob_threshold=0.5, compute_masks=True, min_size=15,
            progress=None, shot_pairs=None, use_transfer_model=True, tsne_diameter=None):
        """The function for the query-evaluate phase."""
        self.postproc_mode = postproc_mode

        if shot_gen is not None:
            self.net.train()
            self.net.phase = 'shot_train'
            if lr is None:
                self.net.lr = {'downsample': 0.001, 'upsample': 0.001, 'tasker': 0.001,
                      'tasker0': 0.0005, 'tasker1': 0.0005, 'tasker_seg': 0.0005,
                      'tasker_class': 0.0005, 'alpha': 0.0005}
            else:
                self.net.lr = lr
            if lr_schedule_gamma is None:
                self.net.lr_schedule_gamma = {'downsample': 0.5, 'upsample': 0.5, 'tasker': 0.5,
                      'tasker0': 0.1, 'tasker1': 0.1, 'tasker_seg': 1.,
                      'tasker_class': 0.1, 'alpha': 0.0005}
            else:
            ...
            # self.net.lr_scheduler = torch.optim.lr_scheduler.StepLR(self.net.optimizer, step_size=100, gamma=0.5)
        ...
        if use_transfer_model:
            preeval_path = r'G:\Python\9-Project\1-flurSeg\scellseg\output\models\preeval'
            preeval_models = [os.path.join(preeval_path, preeval_model) for preeval_model in os.listdir(preeval_path)]
            self.pretrained_model = preeval_models
        ...
        if self.task_mode=='cellpose' or self.task_mode == 'hover':
            if tsne_diameter is not None:
                diameter = tsne_diameter
    ...
    def cellpose_eval(self, imgs, batch_size=8, net_avg=True,
        ...
        if not do_3D:
            ...
            for i in iterator:
                ...
                if compute_masks:
                    ...
                    if self.net.multi_class:
                        cellclassi = transforms.resize_image(y[:, :, 3], shape[-3], shape[-2],
                                                             interpolation=cv2.INTER_NEAREST).astype(np.int32)
                        mask_idx = np.unique(maski)
                        classesi = np.zeros_like(maski, dtype=np.int32)
                        for mask_i in range(1, mask_idx.max()+1):
                            classesi[maski == mask_i] = np.argmax(np.bincount(np.reshape(cellclassi[maski == mask_i], -1)))
                        classes.append(classesi)
        ...
        if self.net.multi_class:
            return (masks, classes), flows, styles
        return masks, flows, styles
    ...
    def classic_eval(self, x, batch_size=8, channels=None, invert=False, normalize=True,
             ...
             tile=True, cell_threshold=None, boundary_threshold=None, min_size=15, shot_pairs=None):
