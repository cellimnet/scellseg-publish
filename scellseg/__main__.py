...
def main():
    parser = argparse.ArgumentParser(description='scellseg parameters')
    ...
    parser.add_argument('--unet', required=False, 
                        default=0, type=int, help='run standard unet instead of scellseg flow output')
    ...
    parser.add_argument('--diameter', required=False, 
                        default=30., type=float, help='cell diameter, if 0 scellseg will estimate for each image')
    ...
    if len(args.dir)==0:
        ...
        if not args.train and not args.train_size:
            ...
            print('>>>> running scellseg on %d images using chan_to_seg %s and chan (opt) %s'%
